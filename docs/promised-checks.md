...
| **게임의 걸음으로 걸어서, 문이 어딘가로 이어지는 모든 건물에 들어간다** | `scripts/viewcheck.mjs: "and a man can walk into every building whose door leads anywhere"` |
| **문이 있는 건물마다 자기 포털에서 나온 바깥 문이 있고, 건물 밖을 향한다** | `scripts/viewcheck.mjs: "every building with a door has a front door out of its own portals, facing out"` |
| 바깥 문이 지붕에 그려진다 | `scripts/viewcheck.mjs: "and the front doors are drawn on the roofs"` |
...
| **다리 바닥이 타일이 아니라 다리가 누운 방향으로 한 장이다** | `scripts/viewcheck.mjs: "a deck is drawn in one piece the way it lies, not a tile at a time"` |
...
| 실내 바닥의 음영 단계가 세 단 이하다 | `scripts/viewcheck.mjs: "and a room is lit flat"` |
...
| 모든 문의 안쪽 칸이 그 실내의 `floor` 다 | `scripts/viewcheck.mjs: "every front door lets a man in on to the room's floor"` |
| 들에서 건물 안쪽 땅이 한 칸도 안 그려진다 | `scripts/viewcheck.mjs: "and no outdoor ground is drawn inside it"` |
| 실내로 옮겨 간 55명이 하나도 사라지지 않는다 (역할별 수 보존) | `scripts/viewcheck.mjs: "and nobody the bake stood under a roof is missing from it, role by role"` |
...
| 세계 캔버스가 `pixelated` 다 | `scripts/padcheck.mjs: "the world canvas is pixelated where the glass scales it, and only there"` |
| 예산표에 전화기 열이 있고 넘긴 것이 없다 | `scripts/budgetcheck.mjs: "and nothing on the table is over what a phone may spend"` |
...
| 그린 바닥 위의 점은 전부 설 수 있다 | `scripts/viewcheck.mjs: "every point the picture paints as floor can be stood on"` |
