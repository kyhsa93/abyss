...
- **Never run `playbot` in the background.** Foreground, blocking, every time. It
  prints as it goes, and a four-minute pull costs four minutes; if that is too long,
  ask for fewer seconds. **Ending your turn ends the session** — there is no later
  to pick anything up in, the tree is committed as it stands, and whatever the run
  was going to say is never read. Four sessions have gone this way, three of them
  consecutively, and the last one's whole output was "I'll wait here for that
  background run to finish before continuing", which is the sentence that ended it.
