...
> 25 heroic → Whisper 5 normal → … → Flasks 25 heroic

Thirty-six kills to open the game. **Clearing a rung opens the one after it, and
...
| 4 | — | the shade | the spore | blood beasts | the flood | the chase |
| 5 | — | the slight | the breath in | the mark | the merging | the flasks |
| 6 | — | the empowered | the breath out | the swallowing | the engulfing | the reagent |
| 7 | — | the turned | — | — | — | — |

The order is the design, not the contents. The first rungs are disjoint across
all six, so the fight everybody sees — the smallest raid, and the first
...
Two of the six ladders are longer than any raid climbs, which is the point of
...
They are one script and six tables (`src/sim/encounters.ts`). A second boss
...
| Mechanic | What it asks of you | Bonegrinder | Whisper | Host | Gorged | Confluence | Flasks |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| Enrage | A hard damage check | 240s | 240s | 240s | 240s | 245s | 250s |
...
Confluence's, and 100 down to 45 across the Two Flasks'. What this row is
catching is the one composition it uses against the five fights that ask for a
