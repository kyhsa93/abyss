...
import { ENCOUNTERS, encounterIndex, type MechanicId } from './sim/encounters'
...
/**
 * What opening a rung buys, in mechanics, which is nothing.
 *
 * The chain's argument used to be that a size and a difficulty each bought a
 * mechanic. The source has no such thing: every setting there throws the whole
 * fight, and what a setting changes is how many people a spell picks, which
 * rank of it lands, and how long the berserk is. So this game's settings do
 * the same, `encounterKit` returns the boss whole, and a rung buys exactly
 * what its name says — more bodies, or heroic.
 *
 * Kept rather than deleted, because the screen that announces a rung still
 * asks, and "nothing new, more of you" is the true answer to say there.
 */
export function rungBuys(_tier: number): MechanicId[] {
  return []
}
